# Model
println("----------------------------------------")
println("Tandem Queue Hitting Time")
println("----------------------------------------")
...
# Lyapunov function
c = 1
w_func(x) = 2c*(x[1]-1.0)+c*(x[2]-1.0)
w_S = vss_func_to_vec(vss, w_func)

closed_form_Qw_func(x) = c*(2ld-mu*((x[1]-1.0)+(x[2]-1.0)))
closed_form_Qw_S = vss_func_to_vec(vss, closed_form_Qw_func)

diff_S = Q*w_S - closed_form_Qw_S
println("Confirm closed form Qw is in agreement Qw: ",
...
@assert is_Lyapunov_function_rate_full_multiply(Q, Acidx_reference, e_S, w_S) 

u_l_reference = fta_lb(G, Aidx_reference, f)
u_u_reference = fta_ub_incomplete(G, Aidx_reference, Acidx_reference, f, w_S)
...
@assert is_Lyapunov_function_rate_full_multiply(Q, Acidx_approx, e_S, w_S) 

u_l_approx = fta_lb(G, Aidx_approx, f)
u_u_approx = fta_ub_incomplete(G, Aidx_approx, Acidx_approx, f, w_S)
...
for (i,t_i) in enumerate(tvals)
  ...
  @assert is_Lyapunov_function_rate_full_multiply(Q, Acidx_i, e_S, w_S)

  u_l_i = fta_lb(G, Aidx_i, f)
  u_u_i = fta_ub_incomplete(G, Aidx_i, Acidx_i, f, w_S)

  rel_err_gap_lower_i = Aidx_to_Sidx(vss, (u_u_reference_S[Aidx_i]-u_l_i)./
                                          u_u_reference_S[Aidx_i], Aidx_i)
  rel_err_gap_upper_i = Aidx_to_Sidx(vss, (u_u_i-u_u_reference_S[Aidx_i])./
                                          u_u_reference_S[Aidx_i], Aidx_i)
