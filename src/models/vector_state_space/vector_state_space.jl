...
function drop_states(Aidx::Vector{Int64}, Acidx::Vector{Int64}, states::Vector{Int64})
    Aidx = setdiff(Aidx, states)
    Acidx = setdiff(Acidx, states)
    return Aidx, Acidx
end
...
function Aidx_to_Sidx(
 ...
 function vss_heatmap(vss::VectorStateSpace, vec_S::Vector; maxsize::Int64 =100, 
    transpose = false, zero_index=true)
    if vss.ndims !=2
        throw(ArgumentError("Heatmap only for 2d vector state space."))
    end
    if zero_index
        if transpose
            p = heatmap(0:maxsize-1, 0:maxsize-1, (vec_to_twodmatrix(vss,vec_S)[1:maxsize,1:maxsize])')
        else
            p = heatmap(0:maxsize-1, 0:maxsize-1, vec_to_twodmatrix(vss,vec_S)[1:maxsize,1:maxsize])
        end
    else
